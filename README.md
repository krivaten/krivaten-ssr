...
- Password: `NghImnMcSrkKm3ff3TcKZ$2*98g34G`
...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly create krivaten-ssr
  fly create krivaten-ssr-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app krivaten-ssr
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app krivaten-ssr-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'krivaten-ssr-staging' does not match app name in config file 'krivaten-ssr'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name krivaten-ssr-db
  fly postgres attach --postgres-app krivaten-ssr-db --app krivaten-ssr

  fly postgres create --name krivaten-ssr-staging-db
  fly postgres attach --postgres-app krivaten-ssr-staging-db --app krivaten-ssr-staging
  ```
